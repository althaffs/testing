 ...
 *  <a href="http://dev_devtoolkit.openmobilealliance.org
 *  /IoT/LWM2M10/doc/">here</a>
 *
...
typedef struct {
	 ...
	 *
	 * \param[in] handle client-specific handle returned by \ref
	 *            client_connect
	 *
	 * \return S_OK on success, error code otherwise
	 */
	 ...
	 * \param[in] buffer pointer to data buffer that should be written to
	 *            the resource
	 ...
	 *
	 * \return S_OK on success, error code otherwise. If the buffer passed
	 *         to hold the read data is too small, E_NO_MEM is returned.
	 */
	 ...
	 * \param[in] user_callback callback function to call upon occurrence of
	 *            the associated event
	 * \param[in] user_data pointer to some user data that will be passed
	 *            back as a parameter to the callback function
	 *
	 * \return S_OK on success, error code otherwise.
	 */
	artik_error(*set_callback)(artik_lwm2m_handle handle,
				artik_lwm2m_event_t event,
				artik_lwm2m_callback user_callback,
				void *user_data);
	/*!
	 * \brief Unset a callback function. It will no longer be called upon
	 *        event occurrence
	 *
	 * \param[in] handle client-specific handle returned by \ref
	 *            client_connect
	 * \param[in] event type of event to unregister the callback function
	 *            from
	 *
	 * \return S_OK on success, error code otherwise.
	 */
	artik_error(*unset_callback)(artik_lwm2m_handle handle,
				artik_lwm2m_event_t event);
	/*!
	 * \brief Create a LWM2M "Device" object to be exposed by the client,
	 *        and initialize it with default values.
	 *
	 * Details about the LWM2M Device object resources can be found
	 * <a href="http://dev_devtoolkit.openmobilealliance.org/IoT/
	 * LWM2M10/doc/TS/index.html#!Documents/lwm2mobjectdevice.htm">
	 *     here
	 * </a>
	 *
	 ...
	 * \param[in] utc_offset Time Zone offset with respect to UTC
	 *            (format: <+|->HH:MM)
	 * \param[in] binding Binding Modes (e.g. "U", "UQ, "S", "SQ")
	 *
	 * \return pointer to the newly created object. When this object is no
	 *         longer used, it must be released by calling \ref free_object.
	 */
	artik_lwm2m_object*(*create_device_object)(const char *manufacturer,
			const char *model,
			const char *serial, const char *fw_version,
			const char *hw_version,
			const char *sw_version,	const char *device_type,
			int power_source,
			int power_volt, int power_current, int battery_level,
			int memory_total,
			int memory_free, const char *time_zone,
			const char *utc_offset,
			const char *binding);
	/*!
	 * \brief Create a LWM2M "Firmware" object to be exposed by the client,
	 *        and initialize it with default values.
	 *
	 * Details about the LWM2M Device object resources can be found
	 * <a href="http://dev_devtoolkit.openmobilealliance.org/IoT/LWM2M10/
	 * doc/TS/index.html#!Documents/lwm2mobjectdevice.htm">
	 *     here
	 * </a>
	 *
	 ...
	 *         longer used, it must be released by calling \ref free_object.
	 */
	artik_lwm2m_object *(*create_firmware_object)(bool supported,
			char *pkg_name,
			char *pkg_version);
	/*!
	 * \brief Create a LWM2M "Connectivity monitoring" object connectivity
	 * monitoring to be exposed by the client, and initialize it with
	 * default values.
	 *
	 * Details about the LWM2M Connectivity monitoring object resources can
	 * be found
	 * <a href="http://dev_devtoolkit.openmobilealliance.org/IoT/LWM2M10/
	 * doc/TS/index.html#!Documents/lwm2mobjectdevice.htm">
	 *     here
	 * </a>
	 *
	 * \param[in] netbearer Indicates the network bearer used for the
	 *            current LWM2M communication session
	 * \param[in] avlnetbearer Indicates list of current available network
	 *            bearer.
	 * \param[in] signalstrength Average value of the received signal
	 *            strength indication used in the current network bearer.
	 * \param[in] linkquality This contains received link quality.
	 * \param[in] lenip Is the size of the array ipaddr .
	 * \param[in] ipaddr The IP addresses assigned to the connectivity
	 *            interface.
	 * \param[in] lenroute Is the size of the array routeaddr .
	 * \param[in] routeraddr The IP address of the next-hop IP router
	 *            (Only two field are read).
	 * \param[in] linkutilization The average utilization of the link to the
	 *            next-hop IP router in %.
	 * \param[in] apn Access Point Name in case Network Bearer Resource is a
	 *            Cellular Network.
	 * \param[in] cellid Serving Cell ID in case Network Bearer Resource is
	 *            a Cellular Network.
	 ...
	 *         longer used, it must be released by calling \ref free_object.
	 */
	artik_lwm2m_object*(*create_connectivity_monitoring_object)
		(
	 ...
	 *
	 * \param[in] object object to release, all its associated memory will
	 *            be released
	 *
	 * \return none
	 */
	 ...
	 * \param[out] buffer Pointer on a array wich it will be allocated with
	 *             the serialized data.
	 * \param[out] lenbuffer Size is the number of cells into the buffer.
	 *
	 * \return S_OK on success, error code otherwise.
	 */
	artik_error(*serialize_tlv_int)(int *data, int size,
