...
typedef struct {
	 ...
	 *
	 *  \param[in,out] handle Handle tied to the requested ADC
	 *                 instance returned by the function.
	 *  \param[in,out] config Configuration to apply to the
	 *                 requested ADC.
	 *
	 ...
	 *
	 *  \param[in] handle Handle tied to the requested ADC
	 ...
	 *             function.
	 *  \param[out] value Filled up with the value of the
	 *              converted analog input
	 *
	 *  \return S_OK on success, error code otherwise
	 */
	artik_error(*get_value) (artik_adc_handle handle,
				int *value);
} artik_adc_module;
