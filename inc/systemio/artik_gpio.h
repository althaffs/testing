...
typedef struct {
	 ...
	 *
	 *  \param[out] handle Handle tied to the requested GPIO instance
	 *              returned by the function.
	 *  \param[in] config Configuration to apply to the requested
	 *             GPIO.
	 *
	 *  \return S_OK on success, error code otherwise
	 */
	artik_error(*request)(artik_gpio_handle * handle,
				artik_gpio_config * config);
	/*!
	 *  \brief Release a GPIO instance
	 *
	 *  \param[in] handle Handle tied to the requested GPIO instance
	 *             to be released.
	 ...
	 *             This handle is returned by the \ref request
	 *             function.
	 *  \param[in] value 1 to set the output active, 0 to set it
	 *             inactive
	 *
	 *  \return S_OK on success, error code otherwise
	 */
	 ...
	 *
	 *  \param[in] handle Handle tied to the requested GPIO instance
	 *             to get the name of. This handle is returned by the
	 *             \ref request function.
	 *
	 *  \return A string of MAX_NAME_LEN maximum length containing
	 *          the user friendly of the GPIO set at request time.
	 ...
	 *
	 *  \param[in] handle Handle tied to the requested GPIO instance
	 *             to get the direction of. This handle is returned
	 *             by the \ref request function.
	 *
	 ...
	 *
	 *  \param[in] handle Handle tied to the requested GPIO instance
	 *             to get the ID of. This handle is returned by the
	 *             \ref request function.
	 *
	 ...
	 *
	 *  \param[in] handle Handle tied to the requested GPIO instance
	 *             from which to monitor changes. This handle is
	 *             returned by the \ref request function.
	 *  \param[in] callback Pointer to the callback function which
	 *             will be called upon GPIO state changes.
	 *  \param[in] user_data Pointer to user data that will be passed
	 *             as a parameter to the callback
	 *
	 *	\return S_OK on success, error code otherwise
	 */
	artik_error(*set_change_callback)(artik_gpio_handle handle,
		artik_gpio_callback callback, void *user_data);
	/*!
	 *  \brief Unset a callback to stop being notified of GPIO state
	 *         change
	 *
	 *  \param[in] handle Handle tied to the requested GPIO instance
	 *             to which the callback was registered. This handle
	 *             is returned by the \ref request function.
	 *
	 */
