...
#if defined(CONFIG_ELOOP_GMAINLOOP)
...
#ifdef CONFIG_ELOOP_GMAINLOOP
int on_watch(int fd, enum watch_io io, void *user_data)
{
	struct eloop_sock *item = user_data;

	log_dbg("fd:%d, io event: %d", fd, io);

	if (item && item->handler)
		item->handler(item->sock, item->eloop_data, item->user_data);
	else
...
#ifdef CONFIG_ELOOP_GMAINLOOP
	log_dbg("add watch: fd=%d", sock);
	loop->add_fd_watch(sock, WATCH_IO_IN | WATCH_IO_ERR | WATCH_IO_HUP |
			WATCH_IO_NVAL, on_watch,
			tmp+table->count, &tmp[table->count].watch_id);
#endif
					...
					int sock)
{
	...
	if (i == table->count)
		return;
#ifdef CONFIG_ELOOP_GMAINLOOP
	if (table->table[i].watch_id > 0) {
		artik_loop_module *loop = (artik_loop_module *)
					artik_request_api_module("loop");
		loop->remove_fd_watch(table->table[i].watch_id);
		table->table[i].watch_id = 0;
		artik_release_api_module(loop);
	}
#endif
...
static void eloop_sock_table_destroy(struct eloop_sock_table *table)
{
	...
	if (table) {
		...
		for (i = 0; i < table->count && table->table; i++) {
			...
			"sock=%d eloop_data=%p user_data=%p handler=0x%"PRIXPTR,
			table->table[i].sock,
			table->table[i].eloop_data,
			table->table[i].user_data,
			(uintptr_t)table->table[i].handler);
			wpa_trace_dump_funcname(
				"eloop unregistered socket handler",
...
{
	log_dbg("register read socket: %d", sock);
			...
			void *eloop_data, void *user_data)
{
	...
	return eloop_sock_table_add_sock(table, sock, handler, eloop_data,
		user_data);
}
			   ...
			   void *eloop_data, void *user_data)
{
	...
	if (timeout->time.sec < now_sec) {
		 ...
		 */
		wpa_printf(MSG_DEBUG, "ELOOP: Too long timeout (secs=%u)\n"
			"to ever happen - ignore it", secs);
		os_free(timeout);
		return 0;
	}
...
static void eloop_handle_alarm(int sig)
{
	wpa_printf(MSG_ERROR, "eloop: could not process SIGINT or SIGTERM in\n"
		"two seconds. Looks like there\nis a bug that ends up in a\n"
		"busy loop that prevents clean shutdown.\n"
...
void eloop_run(void)
{
	...
	while (!eloop.terminate &&
		...
		res = poll(eloop.pollfds, num_poll_fds, timeout ? timeout_ms :
								-1);
#endif /* CONFIG_ELOOP_POLL */
#ifdef CONFIG_ELOOP_SELECT
		...
		res = select(eloop.max_sock + 1, rfds, wfds, efds, timeout ?
				&_tv : NULL);
#endif /* CONFIG_ELOOP_SELECT */
#ifdef CONFIG_ELOOP_EPOLL
		if (eloop.count == 0)
...
void eloop_destroy(void)
{
	...
	struct eloop_timeout, list) {
		...
		wpa_printf(MSG_INFO, "ELOOP: remaining timeout: %d.%06d\n"
			"eloop_data=%p user_data=%p handler=0x%"PRIXPTR,
			sec, usec, timeout->eloop_data, timeout->user_data,
			(uintptr_t)timeout->handler);
